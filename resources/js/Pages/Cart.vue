                        ...
                        <div class="cart-list">
                            <table class="table">
                                ...
                                <tr class="text-center" v-for="cart_item in cart_items" >
                                    ...
                                    <td class="price">{{cart_item.price}}{{cart_item.sign}}</td>
                                    ...
                                    <td class="total">{{cart_item.total}}{{cart_item.sign}}</td>
                                </tr><!-- END TR-->
                ...
                <div class="row justify-content-start">
                    <div class="col col-lg-5 col-md-6 mt-5 cart-wrap ">
                        <div class="cart-total mb-3">
                            ...
                            <p class="d-flex">
                                <span>Số tiền</span>
                                <span>{{subtotal}}{{sign}}</span>
                            </p>
                            <p class="d-flex">
                                <span>Phí giao hàng</span>
                                <span>{{delivery}}{{sign}}</span>
                            </p>
                            <p class="d-flex">
                                <span>Giảm giá</span>
                                <span>{{discount}}{{sign}}</span>
                            </p>
                            <hr>
                        ...
                        <p class="text-center"><inertia-link href="/checkout" class="btn btn-primary py-3 px-4" >Tiến hàng đặt hàng</inertia-link></p>
                    </div>
                </div>
            </div>
            <div class="container" v-else>
                <h2>Giỏ hàng trống</h2>
                <p>Vui lòng chọn sản phẩm trên <inertia-link href="/" >tran sản phẩm</inertia-link>.</p>
            </div>
        </section>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async removeItem(event, product_id) {
                event.preventDefault();
                console.log(this.carts);
                /*
                this.carts = Object.keys(this.carts).filter(product_id_key => {
                    return product_id_key != product_id;
                });
                this.getCart();
                */
               //  console.log(lists);
                this.cart_items_count = this.cart_items_count - this.carts[product_id];
                this.$cookies.set('cart_items_count', this.cart_items_count);
                delete this.carts[product_id];
                this.$cookies.set('carts', this.carts);
