...
class ProductService
{
    ...
    public function getCart($request)
    {
        try {
            ...
            if ($products) {
                ...
                foreach ($products as $id => $product) {
                    if ( isset( $carts[$product['id']] ) ) {
                        ...
                        $products[$id]['total'] = number_format((float) $total_product_price, 0, ',', '.');
            ...
            $total = number_format((float) $subtotal + $delivery - $discount, 0, ',', '.');
            return [
                'success' => true,
                'products' => $products,
                'subtotal' => $subtotal,
    ...
    public function generateSign($request, $product)
    {
        if ( isset($request->currency) && $request->currency == Product::CURRENCY_VND ) {
            $product['price'] = number_format((float) $product['price'] * Product::CURRENCY_VND_RATE, 0, ',', '.');
            $product['sign'] = Product::CURRENCY_VND_SIGN;
        } else {
            $product['sign'] = Product::CURRENCY_USD_SIGN;
        }
        return $product;
    }
